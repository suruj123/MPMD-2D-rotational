...
int main(int argc, char **argv){
  ...
  double radius = 200, angle = 2.0*pi*angle_factor, theta, r, radout, radin; 
  int maxc = 1, minc = 1;
...
//	(int)((rx[n]+regionH[1])/gridWidth[1]) + 1;
	...
	(int)((r)/gridWidth[1]) + 1;
      ...
      Grid[2][c] += 0.5 * ( Sqr(vx[n] - Grid[3][c]) + Sqr(vy[n] - Grid[4][c]) );	
    }*/
...
*/
  ...
  fprintf(fptime,"%lf  %lf  %d %d\n",ke2, 1.0/ke2 ,frn, countgrid);*/
	...
	if(frn == 1){
	printf("nnx = %d nny = %d",nnx,nny);
	}

    //double avvtheta = 0.0, x = 50.0, y = 0.0, cx, cy, y1;
    double avvtheta = 0.0, x, y, cx, cy, y1;

for(ny = 1 ; ny <= ngrids_Y ; ny ++){

  y = (ny-0.5)*gridWidth[2] ;  
  //if(ny>1)
  //y += gridWidth[2];
